...
function disableScript (script) {
  if (script.type !== 'application/x-ajt-script') {
    ...
    script.ajtScriptType = script.dataset.ajtScriptType || ''
  }
}
...
export default async function processContent (doc, options) {
  ...
  if (handlerCallbacks.length > 0) {
    const transision = startViewTransition(() => {
      window.ajtBeforeDomChange && window.ajtBeforeDomChange(options)
      for (let callback of handlerCallbacks) {
        callback()
      }
      window.ajtAfterDomChange && window.ajtAfterDomChange(options)
    })
