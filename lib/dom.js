...
function disableScript (script) {
  if (script.type !== 'application/x-ajt-script') {
    script.ajtScriptType = script.type
    script.type = 'application/x-ajt-script'
  } else {
    script.ajtScriptType = script.dataset.ajtScriptType || 'text/javascript'
  }
}

function replaceScript (original, options) {
  ...
  if (window.ajtNonce) {
    if (options.scriptNonces?.length > 0) {
      for (const nonce of options.scriptNonces) {
...
function handleAddContent (element, options) {
...
export default async function processContent (doc, options) {
  ...
  if (handlerCallbacks.length > 0) {
    ...
    for (let element of addedElements) {
      handleAddContent(element, options)
    }
  }
}
...
if (window.ajtUnsafeScripts || window.ajtNonce) {
  window.ajtElementAddedHandlers.push(function handleScriptNodes (element, options) {
    if (element instanceof HTMLScriptElement) {
      replaceScript(element, options)
    } else if (element instanceof Element) {
      const nodes = element.querySelectorAll('script')
      for (let i = 0, len = nodes.length; i < len; i++) {
        replaceScript(nodes[i], options)
      }
    }
  })
}
